"""
Mixed-Extender OOD Evaluation Set Generation Script

This script generates an "easy OOD" evaluation set of PKS products using
2 extension modules where one uses a training extender code and the other uses
a held-out (OOD) extender code. This produces molecules that are partially
familiar — sharing one extension unit with training data while introducing one
novel unit.

Pipeline:
1. Load ALL 11 extender codes (6 training + 5 OOD)
2. Partition extension modules by AT substrate into train vs OOD
3. Generate all mixed combos (1 train + 1 OOD in both position orderings)
4. Process each bound product through thiolysis and cyclization reactions
5. Deduplicate and remove overlaps with training and existing OOD data
"""
...
# Extender code sets
TRAIN_EXTENDER_CODES = {'Malonyl-CoA', 'Methylmalonyl-CoA', 'allylmal', 'hmal', 'emal', 'mxmal'}
OOD_EXTENDER_CODES = {'butmal', 'hexmal', 'isobutmal', 'D-isobutmal', 'DCP'}
ALL_EXTENDER_CODES = list(TRAIN_EXTENDER_CODES | OOD_EXTENDER_CODES)
...
def partition_modules(all_modules, train_codes, ood_codes):
    """
    Partition structureDB modules into training and OOD sets based on AT substrate.

    Parameters
    ----------
    all_modules : list
        List of bcs.Module objects (structureDB keys)
    train_codes : set
        Set of training extender code names
    ood_codes : set
        Set of OOD extender code names

    Returns
    -------
    Tuple[list, list]
        (train_modules, ood_modules)
    """
    train_modules = []
    ood_modules = []

    for mod in all_modules:
        substrate = mod.domains[bcs.AT].substrate
        if substrate in train_codes:
            train_modules.append(mod)
        elif substrate in ood_codes:
            ood_modules.append(mod)

    return train_modules, ood_modules


def generate_mixed_extension_combos(train_mods, ood_mods):
    """
    Generate all 2-module combos with exactly 1 training + 1 OOD module.

    Both orderings are included:
    - Position 0 = training, position 1 = OOD
    - Position 0 = OOD, position 1 = training

    Parameters
    ----------
    train_mods : list
        List of training extension modules
    ood_mods : list
        List of OOD extension modules

    Returns
    -------
    list
        List of (module_0, module_1) tuples
    """
    combos = []
    # Training first, OOD second
    combos.extend(product(train_mods, ood_mods))
    # OOD first, training second
    combos.extend(product(ood_mods, train_mods))
    return combos


def generate_all_bound_products(mixed_combos) -> List[Tuple]:
    """
    Generate all bound PKS products from mixed extension module combos.
    ...
    """
    # Cross starters with extension combos
    starter_plus_ext_combos = product(bcs.starters.keys(), mixed_combos)

    print(f"\nGenerating clusters and products with mixed 2-module combos...")
    print(f"  Starters: {len(bcs.starters)}")
    print(f"  Extension combos: {len(mixed_combos)}")
    print(f"  Total combinations: {len(bcs.starters) * len(mixed_combos)}")

    with mp.Pool() as pool:
        results = pool.starmap(
            build_bcs_cluster_and_product,
            starter_plus_ext_combos,
            chunksize=CHUNKSIZE_BOUND
        )

    # Filter out failed builds
    results = [r for r in results if None not in r]
    print(f"  Generated {len(results)} bound products")
...
def main():
    """Main entry point for mixed-extender OOD evaluation set generation."""
    ...
    # Partition modules
    all_modules = list(structureDB.keys())
    train_modules, ood_modules = partition_modules(
        all_modules, TRAIN_EXTENDER_CODES, OOD_EXTENDER_CODES
    )
    print(f"Module partition: {len(train_modules)} training, {len(ood_modules)} OOD")

    # Generate mixed combos
    mixed_combos = generate_mixed_extension_combos(train_modules, ood_modules)

    # Output filename
    stereo_suffix = "no_stereo" if REMOVE_STEREOCHEMISTRY else "with_stereo"
    base_filename = f"mixed_ood_pks_products_2_ext_{stereo_suffix}_1train_1ood"

    print("=" * 70)
    print("MIXED-EXTENDER OOD EVALUATION SET GENERATION")
    print("=" * 70)
    print(f"\nConfiguration:")
    print(f"  Training extender codes: {sorted(TRAIN_EXTENDER_CODES)}")
    print(f"  OOD extender codes: {sorted(OOD_EXTENDER_CODES)}")
    print(f"  Training modules: {len(train_modules)}")
    print(f"  OOD modules: {len(ood_modules)}")
    print(f"  Mixed 2-module combos: {len(mixed_combos)}")
    ...
    bound_pks_products = generate_all_bound_products(mixed_combos)
    ...
    print(f"Unique SMILES before overlap removal: {len(unbound_pks_products_dict)}")
    ...
    if train_smiles_files:
        ...
        print("WARNING: No training SMILES files found in output dir — skipping training overlap removal")

    # Remove SMILES that overlap with existing OOD eval sets
    ood_smiles_files = glob.glob(os.path.join(OUTPUT_DIR, "eval_pks_products_*_SMILES.txt"))
    if ood_smiles_files:
        ood_smiles: set = set()
        for fpath in ood_smiles_files:
            with open(fpath) as f:
                ood_smiles.update(line.strip() for line in f if line.strip())
        overlap = set(unbound_pks_products_dict.keys()) & ood_smiles
        if overlap:
            for smi in overlap:
                del unbound_pks_products_dict[smi]
            print(f"Removed {len(overlap)} SMILES overlapping with existing OOD eval data")
        else:
            print("No overlap with existing OOD eval data found")
    else:
        print("No existing OOD eval SMILES files found — skipping OOD overlap removal")

    print(f"Unique SMILES after overlap removal: {len(unbound_pks_products_dict)}")
...
# Load ALL 11 extender codes (6 training + 5 OOD) before importing retrotide
modify_bcs_starters_extenders(starter_codes=STARTER_CODES, extender_codes=ALL_EXTENDER_CODES)
